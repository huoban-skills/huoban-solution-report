...
2. **落位**。在当前工作目录建 `{客户简称}/` 文件夹，复制 assets/template.html 为 `{客户简称}数字化建议报告.html`。
   2.5 **选皮肤**。按客户行业气质推荐一套（给建议＋一句理由，不罗列清单），用户点头后执行 `python3 scripts/apply_skin.py 报告.html 皮肤名`；默认藏蓝金（navy-gold）则跳过。七套皮肤及适用行业见 assets/skins/ 各文件头注释。**与 huoban-image-design 的皮肤同名同气质：报告选定后，配图必须用同名皮肤，报告与图自动同色系。**
